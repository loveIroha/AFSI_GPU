...
长度 cm、时间 s、速度 cm/s、压力 dyn/cm²、rho 为 g/cm³、mu 为 g/(cm·s)、力密度为 dyn/cm³。短程测试的 1 cm 盒子和给定体力用于数值验证，不是左室几何或生理标定。
...
本地 CPU 三步短程测试中，每轴 2/4/8 个单元的压力迭代数分别为 7–9 / 24 / 42–44。Jacobi 提供初始预条件，尚无网格无关收敛保证；尚未实现多重网格，不能据此宣称大规模 GPU 性能已达标。细网格将依据实测迭代数和显存决定后续优化。
...
[本版 Linux CPU 验证已通过](https://github.com/loveIroha/AFSI_GPU/actions/runs/36024695559)，36 组解场最大绝对差 1.71e-10，积分散度也与独立参考一致。目标 CUDA 外部对照仍需以下命令。
